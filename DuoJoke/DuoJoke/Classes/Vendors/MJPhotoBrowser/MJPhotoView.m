...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        [_imageView sd_setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed|SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            NSLog(@"receivedSize = %d",(int)receivedSize);
...
- (void)adjustFrame
{
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        [UIView animateWithDuration:0.3f animations:^{
            NSLog(@"end frame = %@",NSStringFromCGRect(imageFrame));
...
- (void)hide
{
    ...
    [UIView animateWithDuration:duration + 0.1 animations:^{
        NSLog(@"begin _imageView.frame = %@",NSStringFromCGRect(_imageView.frame));
        _imageView.frame = [_photo.srcImageView convertRect:_photo.srcImageView.bounds toView:nil];
        NSLog(@"end _imageView.frame = %@",NSStringFromCGRect(_imageView.frame));
